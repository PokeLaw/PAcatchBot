...
async def pausabot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Sospende gli spawn automatici (solo admin)."""
    global bot_attivo
    ...
    if not bot_attivo:
        await update.message.reply_text("⏸ Il bot è già in pausa.")
        return

    bot_attivo = False
    context.job_queue.scheduler.pause()
    await update.message.reply_text("⏸ Spawn automatici sospesi.")


async def riprendibot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Riattiva gli spawn automatici (solo admin)."""
    global bot_attivo
    ...
    if bot_attivo:
        await update.message.reply_text("▶️ Il bot è già attivo.")
        return

    bot_attivo = True
    context.job_queue.scheduler.resume()
    await update.message.reply_text("▶️ Spawn automatici riattivati.")
...
app.add_handler(CommandHandler("pausabot", pausabot))
app.add_handler(CommandHandler("riprendibot", riprendibot))
